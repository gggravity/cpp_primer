...
public:
  ...
  virtual ~HasPtr() { cout << "destroying has_prt" << endl; }
...
void call_string_blob() { Str_blob sb; }

void call_string_blob_ptr() { Str_blob_ptr sbp; }
...
struct X
{
  X() : x (0), y (0) { cout << "X()" << endl; }

  X (int x, int y) : x (x), y (y) { cout << "X()" << endl; }
