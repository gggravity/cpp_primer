...
public:
  ...
  Sales_data() : _bookNo (""), _units_sold (0), _revenue (0) {}

  explicit Sales_data (const string &book_number) : _bookNo (book_number) {}

  Sales_data (const string &book_number, unsigned units_sold, double price)
      : _bookNo (book_number), _units_sold (units_sold), _revenue (price * units_sold)
  {
  }
...
string Sales_data::isbn() const { return _bookNo; }
