...
struct isbn_mismatch : public logic_error
{
  ...
  isbn_mismatch (const string &s, string lhs, string rhs)
      : logic_error (s), left (std::move (lhs)), right (std::move (rhs)) {
...
public:
  ...
  SalesData &operator+= (const SalesData &other)
  {
    if (isbn() != other.isbn())
      {
        throw isbn_mismatch ("can't add different books", isbn(), other.isbn());
      }
  ...
  static const string SalesData::*data() { return &SalesData::m_book_no; }
...
inline bool compare_ISBN (const SalesData &lhs, const SalesData &rhs) { return lhs.isbn() < rhs.isbn(); }
