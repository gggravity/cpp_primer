...
bulk_quote::bulk_quote (const bulk_quote &other) : discount_quote (other)
{
...
bulk_quote::bulk_quote (bulk_quote &&other) noexcept : discount_quote (std::move (other))
{
...
{
  //    cout << "net_prices from bulk_quote called..." << endl;
  if (count >= quantity)
    {
...
quote *bulk_quote::clone() const & { return new bulk_quote (*this); }

quote *bulk_quote::clone() && { return new bulk_quote (std::move (*this)); }
