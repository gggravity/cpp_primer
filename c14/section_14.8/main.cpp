...
struct if_then_else
    {
        string operator() (bool test, const string &s1, const string &s2)
          {
            return test ? s1 : s2;
          }
    };
...
class Equal
   {
   public:
      explicit Equal (string old_value)
          : old_value(std::move(old_value))
        {

        }

      bool operator() (const string &test_string)
        {
          return test_string == old_value;
        }
