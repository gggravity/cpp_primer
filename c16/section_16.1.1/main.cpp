...
template <typename Iterator, typename T> Iterator find_in (Iterator begin, Iterator end, const T &find)
{
  for (; begin != end; ++begin)
    {
      if (*begin == find)
...
template <typename T, size_t size> void print_t (const T (&array)[size])
{
  for (size_t i = 0; i != size; ++i)
    {
      cout << array[i] << " ";
...
template <typename T, size_t size> T *begin_t (T (&array)[size]) { return array; }

template <typename T, size_t size> T *end_t (T (&array)[size]) { return array + size; }

template <typename T, size_t sz> size_t constexpr size (T (&array)[sz]) { return sz; }
