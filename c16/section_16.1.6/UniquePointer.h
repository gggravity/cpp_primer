...
public:
  ...
  UniquePointer (T *ptr = nullptr, const D &d = D()) noexcept : current_ptr (ptr), deleter (d) {}

  UniquePointer (UniquePointer &&other) noexcept : current_ptr (other.current_ptr), deleter (std::move (other.deleter))
  {
  ...
  T &operator[] (size_t i) const noexcept { return current_ptr[i]; }

  virtual ~UniquePointer() { deleter (current_ptr); }
