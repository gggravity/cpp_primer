...
public:
  ...
  BlobPtr (Blob<T> &a, size_t sz = 0) : w_ptr (a.data), current (sz) {}
  ...
  template <typename U> friend bool operator== (const BlobPtr<T> &lhs, const BlobPtr<T> &rhs);

  template <typename U> friend bool operator!= (const BlobPtr<T> &lhs, const BlobPtr<T> &rhs);

  template <typename U> friend ostream &operator<< (ostream &os, const BlobPtr<T> &ptr);
